...
cd '/Users/rpineau3/Desktop/NEEscripts'
% Low O2
data_low = readtable('data_used_to_generate_figs_in_coexistence_paper.xlsx','Sheet','F2A'); % table
data_low      = data_low{:,:}; % matrix 
% High O2
data_high = readtable('data_used_to_generate_figs_in_coexistence_paper.xlsx','Sheet','F2C'); % table
data_high      = data_high{:,:}; % matrix 
...
for i=1:8
        plot(data_low(:,i),'-','LineWidth',2,'Color', "#969696", ...
...
for i=1:7
        plot(data_high(:,i),'-','LineWidth',2,'Color', "#3288bd", ...
...
ylim([-0.02,1.02]); xlim([1 6]);
xlabel('Days')
ylabel('Proportion of Large clusters','Fontsize',20)
set(gca,'Fontsize',20,'FontName','Avenir');
...
freq_lrg          = data_low(end,lowO2_vec) - data_low(1,lowO2_vec); % calculate the difference between the mean equilibrium frequency and the frequency at start 
freq_sml          = (1-data_low(end,lowO2_vec)) - (1-data_low(1,lowO2_vec)) ;

[out, idx]        = sort(freq_lrg) ;              % Sort (this is to link points in the plot)
freq_at_start     = data_low(1,lowO2_vec) ;
