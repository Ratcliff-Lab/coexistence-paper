...
pdf(file="mean_versus_time.pdf", bg = "transparent",
    width=6, height=4.5, family = "Arial")


par(oma=c(1.2,1,1,1))
par(family="Arial", cex.lab=1.2, cex.axis=1, cex.main=1, cex.sub=1)#, col.lab="white", col.axis = "white")

days <- c(0,5,10,15,20,40) 
clrs <- c(rep("#5ab4ac",3), rep("#d8b365",4))
pchs <- c(rep(21,3), rep(16,4))

plot(1, type="n", xlab="Time (days)", ylab=expression(paste("Mean radius size (", mu, "m)", sep = "")), xlim=c(0, 42), ylim=c(0, 80))
for (p in 1:7){
  
  points(days,means[p,], pch=pchs[p], col = clrs[p])
...
dev.off()
embed_fonts("mean_versus_time.pdf")
